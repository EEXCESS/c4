...
define(['jquery', 'd3', 'QueryCrumbsSettings'], function ($, d3, QueryCrumbsConfiguration) {
    ...
    var self = {
        ...
        RENDERING: {
            ...
            redraw: function (visualData) {
                ...
                var crumbs = self.svgContainer.selectAll("g.crumb").data(visualData, function(d) { return d.queryID;});

                crumbs.exit().attr("opacity",1).transition().duration(500).delay(function(d,i) { return i*50;}).ease("elastic").attr("opacity", 0)
    ...
    return {
         ...
         * @param getHistoryCallback    A callback function, that returns the query history. The function has one parameter {@param numberOfElements} as input and
         * returns an array of {@param numberOfElements} queries. If no function is given, QueryCrumbs retrieves the history from the local storage of the browser.
         ...
         */
        init: function(domElement, getHistoryCallback, navigateQueryCallback) {
            ...
            self.domElem = d3.select(domElement.get(0));
            ...
            if(typeof(getHistoryCallback) !== "function") {
                var getHistoryLS = function(numberOfElements) {
                    if(typeof(window.localStorage) !== "undefined") {
                        var qc = JSON.parse(window.localStorage.getItem("QueryCrumbs"));
                        console.log(qc);
                        return qc.slice(Math.max(qc.length - numberOfElements, 0));
                    } else {
                        return [];
                    }
                };
                getHistoryCallback = getHistoryLS;
            }
            var loadedHistory = getHistoryCallback(QueryCrumbsConfiguration.dimensions.HISTORY_LENGTH);
        ...
        addNewQuery: function(query) {
            ...
            if(typeof(window.localStorage) !== "undefined") {
                window.localStorage.setItem("QueryCrumbs", JSON.stringify(self.historyData));
            }
            self.RENDERING.redraw(self.visualData);
            self.svgContainer.selectAll(".crumb").last().each(function(d, i) {
                self.currentNode = d;
