...
require(['jquery', 'c4/logging'], function($, logging) {
    ...
    $('#moOp').click(function (evt) {
        logging.moduleOpened("anotherModuleName"); return false;
    });
    $('#moCl').submit(function (evt) {
        logging.moduleClosed("anotherModuleName", 475839); return false;
    });
    $('#moSt').submit(function (evt) {
        logging.moduleStatisticsCollected({mystats: "lalelu"}); return false;
    });
    $('#itOp').submit(function (evt) {
        logging.itemOpened(dummyDocumentBadge, dummyQueryID); return false;
    });
    $('#itCl').submit(function (evt) {
        logging.itemClosed(dummyDocumentBadge, dummyQueryID, 8437); return false;
    });
    $('#itCiAsIm').submit(function (evt) {
        logging.itemCitedAsImage(dummyDocumentBadge, dummyQueryID); return false;
    });
    $('#itCiAsTe').submit(function (evt) {
        logging.itemCitedAsText(dummyDocumentBadge, dummyQueryID); return false;
    });
    $('#itCiAsHy').submit(function (evt) {
        logging.itemCitedAsHyperlink(dummyDocumentBadge, dummyQueryID); return false;
    });
    $('#itRa').submit(function (evt) {
        logging.itemRated(dummyDocumentBadge, dummyQueryID, 0, 4, 3); return false;
    });
});